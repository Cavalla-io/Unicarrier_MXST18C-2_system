...
parser.add_argument('-d',  '--daemon',           default=False, action="store_true", help="Run in daemon mode without interactive prompts")
...
with dai.Device(getPipeline()) as device:
    if args.daemon:
        # Just log once and continue running without interactive prompts
        print("UVC device started in daemon mode. /dev/video0 should be available.")
        sys.stdout.flush()
    else:
